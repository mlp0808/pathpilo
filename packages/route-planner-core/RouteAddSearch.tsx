...
interface LocationSuggestion {
  ...
  lng: number
  display: string
  /** True when Mapbox resolved a specific building (house number present). */
  precise: boolean
}

/** A route stop needs a real building pin — street/city centrelines don't count. */
function isPreciseAddress(f: { address?: string; place_name?: string; text?: string }) {
  if (typeof f.address === 'string' && f.address.trim().length > 0) return true
  const streetLine = f.place_name?.split(',')[0] || f.text || ''
  return /\d/.test(streetLine)
}
 ...
 */
export interface RouteAddSearchLabels {
  saved?: string
  newLocation?: string
  searching?: string
  hint?: string
  /** Shown when the user picks a street without a house number. */
  needNumber?: string
}

export interface RouteAddSearchCountry {
  /** ISO 3166-1 alpha-2 (lowercase), or '' for worldwide. */
  code: string
  label: string
  flag: string
}
...
export default function RouteAddSearch({
  ...
  /** Optional i18n overrides for the dropdown microcopy (defaults to English). */
  labels?: RouteAddSearchLabels
  /**
   * When provided, renders a compact country picker inside the field (left of the
   * add button) to scope geocoding. Tool-only — the platform omits it and stays
   * scoped to the company location.
   */
  countryOptions?: RouteAddSearchCountry[]
  ...
  const L = {
    ...
    newLocation: labels?.newLocation ?? 'New location',
    searching: labels?.searching ?? 'Searching…',
    hint: labels?.hint ?? 'Keep typing to find a client or address',
    needNumber: labels?.needNumber ?? 'Add a house number to pin this stop',
  }
  const [query, setQuery] = useState('')
  const [open, setOpen] = useState(false)
  const [locations, setLocations] = useState<LocationSuggestion[]>([])
  const [loading, setLoading] = useState(false)
  const [needNumberHint, setNeedNumberHint] = useState(false)
    ...
    async (text: string) => {
      ...
      try {
        ...
        const res = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${encoded}.json?access_token=${MAPBOX_TOKEN}&autocomplete=true&types=address&limit=8${countryParam}`,
        )
        const data = await res.json()
        // Show every address-type suggestion while typing (street names included).
        // Precision is enforced on pick — filtering here made search look broken
        // because Mapbox autocomplete returns streets first, then numbered buildings.
        const results: LocationSuggestion[] = (data.features || [])
          ...
          .map(
            (f: {
              ...
              const streetText = f.place_name?.split(',')[0]?.trim() || f.text || ''
              const street =
                f.address && !/\d/.test(streetText) ? `${streetText} ${f.address}`.trim() : streetText
              return {
                address: street,
                zip_code: postcode,
                city: place,
  ...
  const pickLocation = (loc: LocationSuggestion) => {
    // Street-only / city-centreline results can't be routed — keep the street
    // in the field and nudge the user to finish with a house number.
    if (!loc.precise) {
      const next = `${loc.address} `.replace(/\s+/g, ' ')
      setQuery(next)
      setNeedNumberHint(true)
      setLocations([])
      setOpen(true)
      inputRef.current?.focus()
      if (debounceRef.current) clearTimeout(debounceRef.current)
      debounceRef.current = setTimeout(() => fetchLocations(next.trim()), 200)
      return
    }
    onPickLocation({
      address: loc.address,
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        ...
        />
        {countryOptions && countryOptions.length > 0 && selectedCountry && (
          <div className="relative flex-shrink-0">
            <button
              type="button"
              onClick={() => setCountryMenuOpen((v) => !v)}
              className="flex items-center gap-1 rounded-full border border-gray-200 bg-white/70 px-2 h-7 text-sm hover:bg-gray-50 transition-colors"
              title={selectedCountry.label}
              aria-label={`Search area: ${selectedCountry.label}`}
              aria-expanded={countryMenuOpen}
            >
              <span className="text-[15px] leading-none">{selectedCountry.flag}</span>
              <svg className={`w-3 h-3 text-gray-400 transition-transform ${countryMenuOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {countryMenuOpen && (
              <div className="absolute right-0 top-full z-[60] mt-2 max-h-72 w-56 overflow-y-auto rounded-2xl border border-gray-200 bg-white py-1.5 shadow-xl">
                {countryOptions.map((c) => {
                  const active = c.code === (countryCode || '')
                  return (
                    <button
                      key={c.code || 'worldwide'}
                      type="button"
                      onClick={() => {
          ...
          <div className="max-h-[50vh] overflow-y-auto py-1.5">
            ...
            {clientMatches.length > 0 && locations.length > 0 && (
              <div className="px-4 pt-2 pb-1">
                <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-gray-300">{L.newLocation}</span>
              </div>
            )}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(loc.zip_code || loc.city) && (
                    <p className="text-[12px] text-gray-400 truncate mt-0.5">
                      ...
                      {!loc.precise ? ` · ${L.needNumber}` : ''}
                    </p>
                  )}
                  {!loc.zip_code && !loc.city && !loc.precise && (
                    <p className="text-[12px] text-amber-600/90 truncate mt-0.5">{L.needNumber}</p>
                  )}
                </div>
                <svg className="w-4 h-4 text-gray-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            ...
            {/* Empty / loading / need-number states */}
            {!hasResults && (
              <div className="px-4 py-6 text-center">
                {loading ? (
                  ...
                  <p className={`text-[13px] ${needNumberHint ? 'text-amber-700' : 'text-gray-400'}`}>
                    {needNumberHint ? L.needNumber : L.hint}
                  </p>
                )}
              </div>
            )}
            {hasResults && needNumberHint && (
              <div className="border-t border-gray-100 px-4 py-2">
                <p className="text-[12px] font-medium text-amber-700">{L.needNumber}</p>
              </div>
            )}
          </div>
        </div>
