...
async function loadAutomationSettings(pool, companyId) {
  const result = await pool.query(
    `SELECT automation_key, enabled, lead_value, lead_unit
     ...
     WHERE company_id = $1
       AND automation_key IN ('email_job_created', 'email_job_reminder')`,
    [companyId]
  );
  const settings = {
  ...
  result.rows.forEach((row) => {
    ...
    settings[row.automation_key] = {
      enabled: !!row.enabled,
      // Confirmation is always minutes (min 1). Reminder is always hours (min 0).
...
async function scheduleAutomationSendsForJob(pool, companyId, jobId) {
  try {
    ...
    // Automation 1: job confirmation
    // Use DB NOW() for send_at so both the insert and the worker comparison (send_at <= NOW())
    // use the same database clock — avoids timezone mismatch with TIMESTAMP WITHOUT TIME ZONE.
    if (settings.email_job_created?.enabled) {
    ...
    // Automation 2: day reminder
    if (settings.email_job_reminder?.enabled && job.scheduled_date) {
      const leadHours =
        settings.email_job_reminder.lead_unit === 'hours'
          ? settings.email_job_reminder.lead_value
          : Math.max(0, Math.ceil(settings.email_job_reminder.lead_value / 60));
      ...
      // Only schedule if the send time is strictly in the future
      if (sendAt && sendAt > now) {
        await pool.query(
          `INSERT INTO scheduled_automation_sends (company_id, job_id, automation_key, send_at)
           VALUES ($1, $2, 'email_job_reminder', $3)
           ON CONFLICT (company_id, job_id, automation_key) DO NOTHING`,
          [companyId, jobId, sendAt]
        );
      }
    }
  } catch (err) {
...
async function rescheduleReminderForJob(pool, companyId, jobId) {
  try {
    ...
    // Remove reminder if job is cancelled or client has no email
    if (job.status === 'cancelled' || !job.client_email) {
      await pool.query(
    ...
    const settings = await loadAutomationSettings(pool, companyId);
    if (!settings.email_job_reminder?.enabled) {
      await pool.query(
        `DELETE FROM scheduled_automation_sends
         WHERE job_id = $1 AND company_id = $2 AND automation_key = 'email_job_reminder'`,
        [jobId, companyId]
...
async function appendAutomationJobLog(pool, jobId, automationKey, clientEmail) {
  try {
    const labelByKey = {
    ...
    const desc = `${label} email sent to ${clientEmail} (${sentAt.toISOString()})`;
    await pool.query(
      `INSERT INTO job_logs (job_id, user_id, action, description) VALUES ($1, NULL, 'automated_email', $2)`,
      [jobId, desc]
    );
  } catch (e) {
...
async function sendForJob(pool, row, automationKey, template) {
  ...
  if (ins.rows.length > 0) {
    await appendAutomationJobLog(pool, row.job_id, automationKey, row.client_email);
  }
}
...
async function getPendingAutomationBadgesForJob(pool, jobId, companyId) {
  ...
  const jobRes = await pool.query(
    `SELECT j.status, COALESCE(NULLIF(TRIM(c.email), ''), '') AS client_email
     FROM jobs j
     JOIN clients c ON c.id = j.client_id
     WHERE j.id = $1 AND j.company_id = $2`,
    [jobId, companyId]
